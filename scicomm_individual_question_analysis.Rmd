...
sc<-sc %>%
   ...
   mutate(community_begin = rowSums(select_(.,"impact_begin", "impact2_begin", "convey_begin", "importscicomm_begin")), 
          ...
          toolsable_begin = sum("tools_begin", "accurateinfo_begin", "assessaccuracy_begin"),
          toolsable_final = sum(tools_final, accurateinfo_final, assessaccuracy_final))
...
scid<-sc %>% 
 select(id, group,scientist_begin:tellstory_begin, scientist_final:tellstory_final, scicommscore_begin:toolsable_final)%>%
  gather(question, value, scientist_begin:toolsable_final)%>%
...
#create vectors of questions within each grouping
community<-c("impact", "impact2", "convey", "importscicomm")
confidence<-c("explanation", "explanation2", "selfconf","tellstory")
identity<-c("commpeer", "commlay", "idscicomm", "commpeer2", "commlay2","idphysiocomm")
toolsable<-c("tools", "accurateinfo", "assessaccuracy")

scid$qgrouping<-ifelse(scid$question %in% community, "community", "NA")
scid$qgrouping<-ifelse(scid$question %in% confidence, "confidence", scid$qgrouping)
scid$qgrouping<-ifelse(scid$question %in% identity, "identity", scid$qgrouping)
scid$qgrouping<-ifelse(scid$question %in% toolsable, "toolsable", scid$qgrouping)
scid$qgrouping<-as.factor(scid$qgrouping)

#run t-tests for each grouping and return p-values
scid %>% 
  filter(survey =="final")%>% #exclude change data for now 
  filter(question %in% qgroupings)%>%
  nest(-question) %>%  #run model on each level of question
...
qgroup_plots<- scid %>% 
  ...
  group_by(qgrouping, group, survey) %>% 
  summarise(smean = mean(value, na.rm = TRUE),  #summary statistics for each Q
            ssd = sd(value, na.rm = TRUE),
            count = n()) %>%
  mutate(se = ssd / sqrt(count),
