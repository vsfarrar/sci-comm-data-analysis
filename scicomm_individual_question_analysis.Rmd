...
```{r not sure what this is..., echo=FALSE, message = FALSE, warning = FALSE}
...
#Q10 summary stats / graph
sc %>% 
  na.omit(assignment) %>%
  group_by(group) %>% 
  summarise(smean = mean(assignment, na.rm = TRUE),  #summary statistics for assignment
            ssd = sd(assignment, na.rm = TRUE),
            count = n()) %>%
  mutate(se = ssd / sqrt(count),
         lower_ci = lower_ci(smean, se, count),
         upper_ci = upper_ci(smean, se, count)) %>%
  group_by(group) %>%
  ggplot(aes(x=group, y= as.numeric(smean), fill = group)) + 
  geom_bar(stat="identity", position = position_dodge(), color = "black")+ 
  geom_errorbar(aes(ymax=upper_ci,ymin=lower_ci),position=position_dodge(0.9), width=0.7)+
  # annotate("text", x = 1, y = 30, label = "n.s.")+ #non-siginifcance labels 
  # annotate("text", x = 2, y = 45, label = "n.s.")+
  ...
  ylim(0,7)+
  scale_fill_manual(name= "Group", labels = c("Control\n(n=16)", "Treatment\n(n=14)"), values = c("grey", "skyblue2"))+ 
  ggtitle("The biweekly assignments were beneficial for my learning")+
  theme_bw()

#Q12 summary stats / graph 
sc %>% 
  na.omit(future_assgn) %>%
  group_by(group) %>% 
  summarise(smean = mean(future_assgn, na.rm = TRUE),  #summary statistics for assignment
            ssd = sd(future_assgn, na.rm = TRUE),
            count = n()) %>%
  mutate(se = ssd / sqrt(count),
         lower_ci = lower_ci(smean, se, count),
         upper_ci = upper_ci(smean, se, count)) %>%
  ...
  ylim(0,7)+
  scale_fill_manual(name= "Group", labels = c("Control\n(n=16)", "Treatment\n(n=14)"), values = c("grey", "skyblue2"))+ 
  ggtitle("I would like to have exercises that are similar to the biweekly assignments in other courses")+
  theme_bw()
```
